...
- 测试基线（2026-09-23 @ HEAD，v2.9.9 发版后追加归档往返断言）：本机 **1217** / `TZ=UTC` **1218** 全绿；v2.9.9 发布门为 1210/1211
- 发布线：**v2.9.9**（插电占比改回醒着口径——v2.9.2 睡眠补记的第三处未门读者；上承 v2.9.8 校准暂停说明、v2.9.7 导出口径元数据、v2.9.6 驻留升降同桶）；队列余 E2（需用户配合）/ H4 / 每轮询成本拆解（触发条件见经验账本）
...
- 目标模式（自主迭代循环）：宪法是根目录 `目标模式·提示词v6.1.md`（本地不入库），细则见 `目标模式作业书.md`（v5）；`迭代报告.md`（每轮流水 + 晨间摘要）与 `经验账本.md`（教训/无恙清单/直觉校准）是其配套本地文件，均 gitignored
