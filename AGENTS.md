...
- 该量具 2026-09-27 实测的三条基线数（后续对比以此为锚，别再重新猜）：静置一次数据发布的级联簇 **~320ms / ~900 轮 layout**（每 2s 一簇 → 静置占空 ~17%）；**起手翻转（false→true）p50 ~30ms**、**停手翻转（true→false）p50 ~200ms**、无事件格 p50 ~3ms；停手那一下里最大项是 `flushPendingUISnapshot` 的那一次发布（消融掉补发：停手窗 p50 200→69ms）。已证伪：探针装卸（不挂探针时静置级联纹丝不动）、逐行淡变（关掉淡变级联照旧 ~900 轮）
