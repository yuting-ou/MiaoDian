...
/// 面板材质三档（v1.24.0 重设计，v1.24.2 更名）：三档映射 Apple 原生液态玻璃（Liquid Glass）的不同真实选项，
/// 而非同一块玻璃上的白纱浓淡。
/// v1.20.0 的教训：三档只差同块 `.regular` 玻璃上的白/黑纱 alpha（浅 0.30→0.74，深三档同参），
/// 被玻璃自身霜感吃掉，肉眼不可辨；深色更是像素级相同——「三种面板状态没有差异」的根源。
/// v1.24.0：材质本体即档位——液态玻璃=`.clear`（原生最透）、均衡=`.regular`（原生标准）、
...
nonisolated enum PanelMaterial: String, Codable, CaseIterable, Sendable, Identifiable {
	case clear       // 液态玻璃：Apple 原生 Liquid Glass（clear），壁纸折射与流动最真实
	...
	var detail: String {
		switch self {
		case .clear: return "Apple 原生 Liquid Glass，壁纸折射与流动最真实"
...
/// 液态玻璃=`.clear`、均衡=`.regular`、厚重=`.regular`+GlassTokens 浓 tint——
