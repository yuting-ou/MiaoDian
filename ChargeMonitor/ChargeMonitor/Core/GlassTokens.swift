...
nonisolated enum GlassTokens {
	...
	/// 卡面是否上真玻璃（v2.0.3 用户裁决「整个面板都要是液态玻璃」）：液态玻璃档=是——
	/// 卡纱浓度进 .clear.tint，磨砂白块在清玻璃壳上读作塑料；均衡/厚重=否（磨砂卡面即档位语义）；
	/// 降低透明度=否（不透明壳上不叠玻璃）。
	nonisolated static func cardIsGlass(_ material: PanelMaterial, reduceTransparency: Bool = false) -> Bool {
		!reduceTransparency && material == .clear
	}
...
// 裁决规则：玻璃让一步，可读性不让步——文本、数字、图表是内容层，永远有 tint 亮度地板托底；
// 玻璃给外壳、控件（悬停/按压有弹性高光）、徽章与仪表（tint 着色）。
// v2.0.3 用户裁决修订：液态玻璃档下「整个面板都要是液态玻璃」——卡面从平涂卡纱升为真玻璃
// （.clear.tint 同浓度），磨砂白块在清玻璃壳上读作塑料；tint 浓度沿用证明表预算，
// 玻璃模糊只降高频不改均值，可读性只增不减。均衡/厚重的磨砂卡面即档位语义，保持平涂。
